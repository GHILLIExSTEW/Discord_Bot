...
def build_pdf():
    ...
    story += section("🧪", "Testing without recording", "Set <b>TESTING=1</b> in .env and configure <b>TEST_CHANNEL_ID</b>, or use the protected command <b>/testing enabled:true</b>. Upload an image in the test channel. The bot parses the image and shows the result without writing to the database, posting a bet, or requiring confirmation. Use <b>Enter units</b> if the caption/image does not show units, then <b>Complete test</b>. Disable live testing with <b>/testing enabled:false</b> or <b>TESTING=0</b>. Only configured operators or server managers can change the runtime toggle; a restart restores the .env default." )
